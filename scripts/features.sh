...
if [[ "$S" == enable ]]; then
  echo "[+] Setting up SukiSU once..."
  curl -LSs "https://raw.githubusercontent.com/SukiSU-Ultra/SukiSU-Ultra/main/kernel/setup.sh" | bash -s builtin

  # SukiSU integration must leave KernelSU/kernel as a real source directory.
  # Remove any stale self-referential nested symlink before Bazel scans the tree.
  if [[ -L "KernelSU/kernel/kernel" ]]; then
    target="$(readlink "KernelSU/kernel/kernel" || true)"
    case "$target" in
      ""|*"/KernelSU/kernel"|../kernel|kernel)
        echo "[!] Removing stale KernelSU/kernel/kernel symlink -> $target"
